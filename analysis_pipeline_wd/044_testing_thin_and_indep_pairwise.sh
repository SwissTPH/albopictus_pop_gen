...
#SBATCH --qos=30min
#SBATCH --output=logs/test_thinning.out
#SBATCH --error=logs/test_thinning.err
...
FILTER_INDV=042_filter_${EXP}_MD_30/${EXP}FMi_0.20.in.txt
FILTER_NATIVE=042_filter_${EXP}_MD_30/${EXP}FMi_0.20.in.native.txt
awk '{print $1"_"$1" "$1"_"$1}'  $FILTER_NATIVE > $OUT_FOLDER/tmp.filter_native

FILTER_INVADED=042_filter_${EXP}_MD_30/${EXP}FMi_0.20.in.invaded.txt
awk '{print $1"_"$1" "$1"_"$1}'  $FILTER_INVADED > $OUT_FOLDER/tmp.filter_invaded

## thinning and 
## filtering individuals based on fraction missing

VCF_OUT=${OUT_FOLDER}/populations.snps.filter_FM_${EXP}_MD_30.FMi_0.2_thin400
...
VCF_OUT=${OUT_FOLDER}/populations.snps.filter_FM_${EXP}_MD_30.FMi_0.2_LD_thin
