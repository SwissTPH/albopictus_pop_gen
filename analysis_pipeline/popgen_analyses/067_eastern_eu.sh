#!/bin/bash
#SBATCH --job-name=067_eastern_europe
#SBATCH --mem=8G
#SBATCH --cpus-per-task=1
#SBATCH --time=00:30:00
#SBATCH --qos=30min
#SBATCH --output=/scicore/home/muellepi/marmor0000/albopictus_ddRADseq/popgen_analyses/PCA/067_eastern_europe/err_out/logs.out
#SBATCH --error=/scicore/home/muellepi/marmor0000/albopictus_ddRADseq/popgen_analyses/PCA/067_eastern_europe/err_out/logs.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=sarah.marmorosch@swisstph.ch

# load modules
module load PLINK/1.90-beta-7.6-x86_64

VCF=/scicore/home/muellepi/marmor0000/albopictus_ddRADseq/snp_filtering/040_FMi_and_LD_filter/FM_0.65_mD_3_MD_30_FMi_0.3/FM_0.65_mD_3_MD_30_FMi_0.3_LD_thin.vcf.gz
OUT_FOLDER=/scicore/home/muellepi/marmor0000/albopictus_ddRADseq/popgen_analyses/PCA/067_eastern_europe
POPMAP=/scicore/home/muellepi/marmor0000/albopictus_ddRADseq/popmap_albo.txt

#create keep file with populations to include in PCA
awk '($2=="Albania" || $2=="Bulgaria" || $2=="Croatia" || $2=="Greece" || $2=="Italy" || $2=="Montenegro" || $2=="Serbia" || $2=="Slovenia" || $2=="TurkeyWest" || $2=="TurkeyEast" || $2=="Israel" || $2=="Georgia") {print $1, $1}' \
$POPMAP > ${OUT_FOLDER}/pops_eastern_europe.keep

# PCA
# prepare PLINK files once
plink --vcf ${VCF} \
      --double-id \
      --allow-extra-chr \
      --set-missing-var-ids @:# \
      --make-bed \
      --out ${OUT_FOLDER}/LD_thin_pops_eastern_europe

# PCA
plink --bfile ${OUT_FOLDER}/LD_thin_pops_eastern_europe \
      --double-id \
      --allow-extra-chr \
      --keep ${OUT_FOLDER}/pops_eastern_europe.keep \
      --pca 300 \
      --out ${OUT_FOLDER}/FM_0.65_mD_3_MD_30_FMi_0.3_LD_thin_eastern_europe


